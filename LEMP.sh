...
sleep 15
#Add the Autoscaling policy to the AutoScaling Group
configFilePath="$PWD/myLEMPApplicationSGPolicy.json"
aws autoscaling put-scaling-policy --policy-name myLEMPApplicationSGPolicy --auto-scaling-group-name myLEMPApplicationSG --policy-type TargetTrackingScaling --estimated-instance-warmup 100 --target-tracking-configuration file://$configFilePath
